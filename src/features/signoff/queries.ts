...
export async function getSignoffDetail(id: string): Promise<SignoffDetail | null> {
  ...
  const { data } = await supabase
    ...
    .select(`id, reporting_periods ( year, label )`)
    .eq("id", id)
    .maybeSingle()
    .returns<{ id: string; reporting_periods: { year: number; label: string } | null } | null>();

  if (!data?.reporting_periods) return null;
 ...
 *
 * The dashboard has no department of its own — every query on it is
 * department-agnostic and RLS decides what the numbers cover. That makes the
 * page an organisation-wide aggregate for anyone holding no department role
 * (IMS admins, reviewers, viewers) and a two-department pool for anyone
 * holding two. Sign-off is per department, so it is shown only where exactly
 * one department is unambiguous; everywhere else the header stays as it was.
 */
...
export async function getHeaderSignoff(
  ...
  const departmentId = soleDepartmentId(user);
